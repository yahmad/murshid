...
Murshid uses a multi-tiered file monitoring design in [watcher.rs](file:///Users/yasir/src/thabit/murshid/src/watcher.rs) to process code edits recursively.

### Resource Throttling Coordinator
To prevent system crashes due to OS descriptor limits (such as `EMFILE` or `ENOSPC`), the coordinator in [watcher_coordinator.rs](file:///Users/yasir/src/thabit/murshid/src/watcher_coordinator.rs) tracks active watch threads and file descriptor counts process-wide:
...
When a file edit event is validated, the watcher triggers the compiler interceptor in [compiler.rs](file:///Users/yasir/src/thabit/murshid/src/compiler.rs).
