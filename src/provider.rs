...
fn format_turn_content(turn: &DialogueTurn) -> String {
    if let Some(ref code) = turn.code_context {
        format!("Error Code: {}\nCode Context:\n{}\nMessage: {}", turn.error_code, code, turn.message)
    } else {
...
fn run_query_with_child_tracking(
    ...
    if let Some(child) = child_opt {
        let output = child.wait_with_output().map_err(|e| format!("curl execution failed: {}", e))?;
        if output.status.success() {
            let res_str = String::from_utf8(output.stdout).map_err(|e| format!("Invalid UTF-8 response: {}", e))?;
...
mod tests {
    ...
    fn test_formatting_templates() {
        ...
        assert!(llama3.starts_with("<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\nSystem rules<|eot_id|>"));
    ...
    fn test_debounce_cancellation() {
        ...
        let handle1 = std::thread::spawn(|| {
            dispatch_debounced("ollama", "prompt 1", None)
        });
        ...
        let handle2 = std::thread::spawn(|| {
            dispatch_debounced("ollama", "prompt 2", None)
        });
