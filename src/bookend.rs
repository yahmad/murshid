...
mod tests {
    ...
    fn test_assemble_bookend_goal_line_with_goal() {
        let b = assemble_bookend(Some("fix auth timeout"), BookendCounts::default(), vec![], vec![], vec![]);
        assert_eq!(b.goal_line, "goal: fix auth timeout");
    }

    #[test]
    fn test_assemble_bookend_goal_line_without_goal() {
        let b = assemble_bookend(None, BookendCounts::default(), vec![], vec![], vec![]);
        assert_eq!(b.goal_line, "goal: (none set)");
        let b2 = assemble_bookend(Some("   "), BookendCounts::default(), vec![], vec![], vec![]);
    ...
    fn test_assemble_bookend_truncates_queue_last_call_to_three() {
        ...
        let b = assemble_bookend(None, BookendCounts::default(), vec![], vec![], queue);
