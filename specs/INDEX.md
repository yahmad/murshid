...
| T17 | Passive learning hint-stream (redesign round 3) | **Done — MERGED to main 2026-07-09** (`f4db7f7`; all 7 rungs R0–R6 implementer→adversarial-gate PASS; 942+1 tests, clippy clean). Min-gap cooldown replaces the frequency detent; silent-when-healthy; the spaced learning memory (cross-session dismiss/ignore suppression + resurfacing, migration 14); the always-hint flip (consent deleted, real advice_fp, `👍 useful` key); history-stream home; split-pane detail + ask-any-card; events split-pane + slim chrome + reserved-`?` 1-row keybar. Dogfood findings 2026-07-09 + the Working Backwards gap analysis feed T19. | [T17-passive-hint-stream.md](tasks/T17-passive-hint-stream.md) |
| T19 | "Legible murshid" — Working Backwards UX queue | **Drafted 2026-07-09** — founder decisions taken (g→spaced window; startup quiz removed; one-provider default): out-of-box path, live judge health, self-teaching response keys, visible memory/graduation, model tolerance, silence legibility. | [T19-legible-murshid.md](tasks/T19-legible-murshid.md) |
| T18 | Pack pipeline (bootstrap + enrichment) + Python & TS packs | **Drafted 2026-07-09** — founder direction given; builds in parallel on branch `yasir/t18-packs` (zero file overlap with T17). R0 validator + pipeline docs and R1 Rust-taxonomy enrichment are dep-free and may build now; R2 (Python) / R3 (TypeScript) gated on ratifying the proposed C10 blanket amendment (pack-scoped `tree-sitter-{lang}` crates). | [T18-pack-pipeline.md](tasks/T18-pack-pipeline.md) |

**Active spec for current work:** T19 (Legible murshid — next ladder, not yet started) with T18 (pack pipeline, branch `yasir/t18-packs`: R0–R1 done, R2/R3 awaiting C10 tree-sitter ratification) in parallel. T17 merged 2026-07-09.
