...
| T17 | Passive learning hint-stream (redesign round 3) | **In progress** — founder go 2026-07-07; building serial+gated on branch `yasir/t17-hint-stream`. R0–R3 done + gated (min-gap cooldown; silent-when-healthy; spaced learning memory incl. migration 14; always-hint flip + `👍 useful` key, real advice_fp, R2 gate wired). R4–R6 remain (history-as-home, split-pane detail+ask, events split+slim chrome). Amends I9/I10/D10/D16 + surfacing. | [T17-passive-hint-stream.md](tasks/T17-passive-hint-stream.md) |
...
**Active spec for current work:** T17 (redesign round 3, branch `yasir/t17-hint-stream`, R4–R6 in flight) with T18 (pack pipeline, branch `yasir/t18-packs`) running in parallel. Neither merged yet.
